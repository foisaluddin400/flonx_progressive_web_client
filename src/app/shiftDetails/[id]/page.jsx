...
const Page = () => {
  ...
  const { data: myOrder, refetch } = useGetMyOrderQuery(
          ...
          {orders.map((order) => (
            <div
              ...
              className={`${getBgColor(
                order.status,
              )} text-white rounded-2xl p-4 mb-5`}
            >
              {/* HEADER */}
              ...
              <div className="flex border-t border-white/40 pt-3 gap-3 mt-4 flex-col">
                ...
                {order.status === "QUEUED" && (
                  <div className="flex gap-3">
                    ...
                    <button
                      onClick={() =>
                        handleUpdateStatus(order._id, "IN_PROGRESS", "start")
                      }
                      disabled={loadingId === order._id}
                      className={`flex-1 py-2 rounded-full flex justify-center items-center gap-2 ${
                        loadingId === order._id
                ...
                {order.status === "IN_PROGRESS" && (
                  ...
                  >
                    ...
                    <div
                      ...
                      onTouchMove={(e) =>
                        handleMove(e.touches[0].clientX, order._id)
                      }
                      style={{
                        left: sliderState[order._id]?.position || 0,
                        transition: sliderState[order._id]?.dragging
                          ? "none"
                          : "all 0.3s ease",
                      }}
